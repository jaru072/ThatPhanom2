  ...
  L.siteSections.forEach(sec => {
    ...
    if (hasTitle) {
      if (sec.id === "voices") {
        ...
        let hTitle = null;
        if (sectionEl.classList.contains("milestones-overview-media")) {
        ...
        if (hTitle) {
          ...
          if (i18nKey && window.I18N_TH) window.I18N_TH[i18nKey] = targetTitle;
        }
      }
    }
    ...
    if (hasKicker) {
      if (sec.id === "footer") {
        ...
        const kickerEl = sectionEl.querySelector(".milestone-card-copy time, .milestone-card-copy .kicker, .timeline-head .kicker, .section-heading .kicker, .section-head .kicker, .hero-copy .eyebrow, .kicker, .eyebrow, time");
        if (kickerEl) {
          kickerEl.textContent = targetKicker;
          const i18nKey = kickerEl.getAttribute("data-i18n");
          if (i18nKey) {
            if (window.I18N_TH) window.I18N_TH[i18nKey] = targetKicker;
            if (window.I18N_LANGS) {
              if (window.I18N_LANGS.th?.dict) window.I18N_LANGS.th.dict[i18nKey] = targetKicker;
              if (curLang === "lo" && window.I18N_LANGS.lo?.dict) window.I18N_LANGS.lo.dict[i18nKey] = targetKicker;
              if (curLang === "en" && window.I18N_LANGS.en?.dict) window.I18N_LANGS.en.dict[i18nKey] = targetKicker;
            }
          }
        }
      }
    ...
    if (hasContent) {
      if (sec.id === "footer") {
        ...
        if (sectionEl.id === "muchalinda-history" || sectionEl.classList.contains("muchalinda-overview-media") || sectionEl.classList.contains("milestones-overview-media")) {
          ...
          descEl = sectionEl.querySelector(".milestone-card-copy span, .milestone-card-copy p, .process-step-copy span, .process-step-copy p, .criteria-card-copy p, .story-feature-copy p, .story-mini-copy p, .hero-copy p.lead, p.lead, .section-description");
        }
        ...
        if (descEl) {
          ...
          if (i18nKey) {
            if (window.I18N_TH) window.I18N_TH[i18nKey] = targetContent;
            if (window.I18N_LANGS) {
              if (window.I18N_LANGS.th?.dict) window.I18N_LANGS.th.dict[i18nKey] = targetContent;
              if (curLang === "lo" && window.I18N_LANGS.lo?.dict) window.I18N_LANGS.lo.dict[i18nKey] = targetContent;
              if (curLang === "en" && window.I18N_LANGS.en?.dict) window.I18N_LANGS.en.dict[i18nKey] = targetContent;
            }
          }
        }
...
function _openSectionEditor(sectionTargetId) {
  ...
  const secData = (L.siteSections || []).find(s => s.id === sectionTargetId) || (sectionTargetId === "muchalindaCardHistory" ? (L.siteSections || []).find(s => s.id === "muchalindaHistory") : null) || (sectionTargetId === "muchalindaCardRestoration" ? (L.siteSections || []).find(s => s.id === "muchalindaFacts") : null) || {};
  ...
  let currentTitle = typeof secData.title === "string" ? secData.title : "";
  let currentKicker = typeof secData.kicker === "string" ? secData.kicker : "";
  let currentContent = typeof secData.content === "string" ? secData.content : "";
  let currentImage = typeof secData.imageUrl === "string" ? secData.imageUrl : "";
  let currentAction = typeof secData.actionUrl === "string" ? secData.actionUrl : "";
  ...
  if (sectionEl) {
    ...
    if (secData.kicker === undefined) {
      if (sectionTargetId === "footer") {
        ...
        currentKicker = sectionEl.querySelector(".milestone-card-copy time, .milestone-card-copy .kicker, .timeline-head .kicker, .section-heading .kicker, .section-head .kicker, .hero-copy .eyebrow, .kicker, .eyebrow, .hero-kicker, .status-label, time, .feature-no, .criteria-number")?.textContent || "";
      }
    }
    if (secData.content === undefined) {
      if (sectionTargetId === "footer") {
      ...
      } else if (sectionEl.id === "muchalinda-history" || sectionEl.classList.contains("muchalinda-overview-media") || sectionEl.classList.contains("milestones-overview-media")) {
        ...
        currentContent = sectionEl.querySelector(".milestone-card-copy span, .milestone-card-copy p, .process-step-copy span, .process-step-copy p, .criteria-card-copy p, .story-feature-copy p, .story-mini-copy p, .hero-copy p.lead, p.lead, .section-description")?.textContent || "";
      }
    }
    if (secData.imageUrl === undefined) {
      currentImage = sectionEl.querySelector("img")?.src || "";
    }
    if (secData.actionUrl === undefined) {
