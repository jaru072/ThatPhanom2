...
${t}`)&&n();return}i("#confirmTitle").textContent=e,i("#confirmMessage").textContent=t;const r=i("#confirmOk");r&&(r.textContent=o),a.open&&a.close(),a.showModal();const s=()=>{a.open&&a.close()};i("#confirmCancel").onclick=s,i("#confirmOk").onclick=async()=>{i("#confirmOk").disabled=!0;try{await n()}finally{i("#confirmOk").disabled=!1,s()}}}function xe(e,t,n){if(n==="lo"){const o=e[`${t}_lo`];if(o&&String(o).trim())return o}else if(n==="en"){const o=e[`${t}_en`];if(o&&String(o).trim())return o}return e[t]||""}const _MUCHALINDA_DEFAULT_RAIL_CARDS = [
  {
    ...
    title: "บูรณะสระมุจลินท์ (สระพังทอง)",
    title_lo: "ບູລະນະສະມຸດຈະລິນ (ສະພັງທອງ)",
...
function Ue(){
  ...
  if (isMucha) {
    // Return items scoped to muchalinda, plus defaults if none exist
    const muchaCards = rawCards.filter(c => c.projectScope === "muchalinda-restoration" || c.projectScope === "muchalinda");
    if (muchaCards.length > 0) {
      return muchaCards.sort((a,b) => (Number(a.order)||0) - (Number(b.order)||0));
    }
    return _MUCHALINDA_DEFAULT_RAIL_CARDS.slice().sort((a,b) => (Number(a.order)||0) - (Number(b.order)||0));
  }
...
function je(){
  ...
  if (isMucha) {
    ...
    if (muchaItems.length > 0) {
      return muchaItems.sort((e,t)=>(Number(e.order)||0)-(Number(t.order)||0));
    }
    return _MUCHALINDA_DEFAULT_SIDEBAR_ITEMS.slice().sort((e,t)=>(Number(e.order)||0)-(Number(t.order)||0));
  }
  ...
  if (mainItems.length > 0) {
    return mainItems.sort((e,t)=>(Number(e.order)||0)-(Number(t.order)||0));
  }
  return _WORLD_HERITAGE_DEFAULT_SIDEBAR_ITEMS.slice().sort((e,t)=>(Number(e.order)||0)-(Number(t.order)||0));
}
